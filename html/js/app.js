(function () {
    let MenuTpl =
        '<div id="menu_{{_namespace}}_{{_name}}" class="menuConfirm">' +
        '<div class="menuTitles"><div class="menuTitle">{{title}}</div>' +
        '<div class="menuDesc">{{titleModal}}</div></div>' +
        '<div class="boxInput"><input type="text" name="" id="" placeholder="Wprowadź tekst" class="inputValue"></div>' +
        '<div class="boxBtns"><div class="btn cancel">Anuluj</div><div class="btn confirm">Potwierdź</div></div></div>';
    window.ESX_MENU = {};
    ...
    ESX_MENU.render = function () {
        ...
        $(menuContainer).find('.btn.confirm').unbind("click");
        $(menuContainer).find('.btn.cancel').unbind("click");
        $(menuContainer).find('.inputValue').unbind("input propertychange");
